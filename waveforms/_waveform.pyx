...
cdef NativeCore _wrap_wave(cwaveform_wave *pointer):
    if pointer == NULL:
...
cdef NativeStackCore _wrap_stack(cwaveform_stack *pointer):
    if pointer == NULL:
...
cdef NativeSamplePlan _wrap_plan(cwaveform_sample_plan *pointer):
    if pointer == NULL:
        return None
    cdef NativeSamplePlan result = NativeSamplePlan.__new__(NativeSamplePlan)
    result._pointer = pointer
    return result
...
cdef class NativeCore:
    ...
    def __eq__(self, other):
        return isinstance(other, NativeCore) and cwaveform_wave_equal(
...
cdef class NativeSamplePlan:
    ...
    def sample(self, double offset=0.0, int bits=0,
        ...
        if status != 0:
            raise RuntimeError(
                f"native sample plan failed with status {status}"
            )
        return target
