...
# For backward compatibility, keep the old PLY-based parser available
# This can be removed once we're confident the ANTLR version works perfectly
try:
    import tempfile
    ...
    class _WaveLexer:
        """Legacy PLY-based Waveform Lexer."""
    ...
    class _WaveParser:
        """Legacy PLY-based Waveform Parser."""
...
def wave_eval(expr: str, use_antlr: bool = True) -> "waveform.Waveform":
    """
    Parse and evaluate a waveform expression.
    
    Args:
        expr: The expression string to parse
        use_antlr: Whether to use ANTLR parser (default) or fall back to PLY
    ...
    if use_antlr:
        try:
            ...
            # If ANTLR fails and PLY is available, try PLY as fallback
            if PLY_AVAILABLE and _wave_parser is not None:
                try:
            ...
            if PLY_AVAILABLE and _wave_parser is not None:
                try:
                    ...
                    pass
            raise SyntaxError(f"Illegal expression '{expr}': {str(e)}")
    else:
        # Use legacy PLY parser
        if not PLY_AVAILABLE or _wave_parser is None:
            raise SyntaxError("PLY parser not available. Please install ply package or use ANTLR parser.")
        try:
            return _wave_parser.parse(expr)
        except:
